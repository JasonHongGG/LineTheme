...
class ThemeWorkbenchController extends ChangeNotifier
    with WidgetsBindingObserver {
  ThemeWorkbenchController({LineThemeRepository? repository})
    : _repository = repository ?? LineThemeRepository() {
  ...
  final TextEditingController urlController = TextEditingController(
    text:
        'https://store.line.me/themeshop/product/63d469be-e4c4-46d9-8822-bc8ddcc2fb0f/zh-Hant',
  );
  ...
  Future<void> bootstrap() async {
    ...
    try {
      ...
      _updateState(
        _state.copyWith(shizukuStatus: status, hasFileAccess: status.isReady),
      );
  ...
  Future<void> requestAccess() async {
    ...
    try {
      ...
      if (granted && status.isReady) {
        ...
        _updateState(
          _state.copyWith(errorMessage: _buildShizukuGuidance(status)),
        );
      }
    } catch (error) {
      _updateState(_state.copyWith(errorMessage: error.toString()));
  ...
  Future<void> reloadInstalledThemes() async {
    ...
    try {
      ...
      _updateState(
        _state.copyWith(installedThemes: themes, selectedSlot: selectedSlot),
      );
    } catch (error) {
      _updateState(_state.copyWith(errorMessage: error.toString()));
    } finally {
  ...
  Future<void> resolveTheme() async {
    ...
    if (sourceUrl.isEmpty) {
      ...
      if (clipboardText.isEmpty) {
        _updateState(
          _state.copyWith(
            errorMessage: '請先輸入主題網址，或先把網址複製到剪貼簿。',
            progressValue: 0,
            progressMessage: '等待網址',
          ),
        );
        return;
      }
    ...
    try {
      ...
      _updateState(
        _state.copyWith(
          ...
          progressValue: 0.22,
          progressMessage: '已取得主題資訊與版本',
          activityLog: _prependLog(
            _state.activityLog,
            '找到 Theme ID ${info.themeId} / version ${info.version}',
          ),
        ),
      );
    } catch (error) {
  ...
  Future<void> applyTheme() async {
    ...
    try {
        ...
        onProgress: (event) {
          _updateState(
            _state.copyWith(
              ...
              activityLog: event.logLine == null
                  ? _state.activityLog
                  : _prependLog(_state.activityLog, event.logLine!),
            ),
          );
        },
      );
      ...
      _updateState(
        _state.copyWith(
          progressValue: 1,
          progressMessage: '已完成並覆寫 themefile.$selectedSlot',
          activityLog: _prependLog(
            _state.activityLog,
            'themefile.$selectedSlot 已完成更新',
          ),
        ),
      );
    } catch (error) {
      _updateState(
        _state.copyWith(
          errorMessage: error.toString(),
          activityLog: _prependLog(_state.activityLog, '處理失敗: $error'),
        ),
      );
    } finally {
